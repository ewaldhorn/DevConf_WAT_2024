...
WebAssembly.instantiate(rustBuffer).then((wasmModule) => {
  ...
  console.log(
    `Rust : Turns out, adding ${left} to ${right} gives ${add(
      left,
      right
    )}! Who'd have thunk?`
  );
...
WebAssembly.instantiate(goBuffer).then((wasmModule) => {
  ...
  const value = 111;
  console.log(
    `Go   : If you double ${value}, you get ${double(value)}. No jokes.`
  );
});
...
WebAssembly.instantiate(zigBuffer).then((wasmModule) => {
  ...
  const right = 2;
  console.log(
    `Zig  : If you multiply ${left} with ${right}, you get ${product(
      left,
      right
    )}. No cap.`
  );
...
WebAssembly.instantiate(cBuffer).then((wasmModule) => {
  ...
  const value = 4;
  console.log(
    `C    : If you square ${value}, you get ${square(value)}. Wasted.`
  );
});
